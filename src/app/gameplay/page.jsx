...
const GamesPage = async ({searchParams}) => {
  ...
  return (
    <div className="mt-12">
      ...
      <div className="grid grid-cols-3 md:grid-cols-3 gap-3 md:gap-4 mt-8 px-4 md:px-0">
        {allGames.length > 0 &&
          allGames.map((game) => (
            <div
              key={game._id}
              className="relative h-24 md:h-56 bg-cover bg-center shadow-md shadow-slate-200 rounded-md"
            ...
            >
              <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 hover:opacity-100 bg-black bg-opacity-0 hover:bg-opacity-75 transition-opacity duration-300 rounded-md">
                <h1
                  className={`${quantico.className} text-white text-center text-sm md:text-lg transition-opacity duration-300`}
                >
                  {game.name}
                </h1>
                <Link
                  href={`/gameplay/${game._id}`}
                  className={`${quantico.className} text-sm font-semibold transition-opacity duration-300 mt-2 my-1 md:py-2 px-2 md:px-3 bg-emerald-600 text-white rounded-md md:rounded-lg`}
                >
                  Game Details
                </Link>
              </div>
